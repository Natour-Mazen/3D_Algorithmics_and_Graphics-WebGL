vec3 phongLighting(float uLightShininess, vec3 lightDir, vec4 uLightSpecular,
                   vec4 uLightColor, vec4 Color, float uPI, float weight, vec3 normal) {
    ...
    return uLightColor.rgb * ((1. - uLightSpecular.rgb) * Color.rgb + ((8. + n) / (8. * uPI)) * uLightSpecular.rgb * weightSpec) * weight;
}
